...
func _ready():
	...
	if Network.isServer:
		...
		if saveDict == null || saveDict.empty():
			...
			if true:
				var theOutput = []
				var exit_code = OS.execute(".", ["cd " + OS.get_user_data_dir() + "; git init"], true, theOutput)
			...
			var theOutput = []
			var exit_code = OS.execute(".", ["cd " +  OS.get_user_data_dir() + " ; echo bash may have worked; git add .; git commit -m \"added initial save files\";  ls ."], true, theOutput)
